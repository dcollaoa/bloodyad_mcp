...
set_mcp_configuration() {
    ...
    if grep -q "bloodyad-mcp:" "$registry_yaml"; then
        ...
        if grep -q "^registry:" "$registry_yaml"; then
            sed -i "" "/"^registry:""/a \n  bloodyad-mcp:\n    ref: \"\"" "$registry_yaml" # macOS sed requires an empty string for -i
        else
            echo -e "\nregistry:\n  bloodyad-mcp:\n    ref: \"\"" >> "$registry_yaml"
        fi
...
update_gemini_settings() {
    ...
    if ! command -v jq &> /dev/null; then
        ...
        write_status "On macOS: brew install jq" "Info"
...
show_settings_json_sample() {
    local user=$(whoami)
    local mcp_path="${HOME}/.docker/mcp" # Corrected for macOS
